...
import { ChevronLeft, ChevronRight, Code2, ListChecks, Search, Timer, Zap } from "lucide-react"
...
const difficultyConfig: Record<string, { tone: "green" | "amber" | "red"; stripe: string }> = {
  easy: { tone: "green", stripe: "bg-emerald-500" },
  medium: { tone: "amber", stripe: "bg-amber-500" },
  hard: { tone: "red", stripe: "bg-rose-500" },
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <SectionHeading
        ...
        description="Free problems from past contests. Filter by difficulty and type, then solve at your own pace."
      />
      ...
      {data && (
        <div className="mt-6 flex items-center gap-4 text-sm text-muted-foreground">
          <span className="flex items-center gap-1.5">
            ...
            {activeCount} problem{activeCount !== 1 ? "s" : ""}
          </span>
          <span className="flex items-center gap-1.5">
            <Code2 className="h-4 w-4 text-blue-500" />
            {data.problems.filter((p) => p.type === "coding").length} coding
          </span>
          <span className="flex items-center gap-1.5">
            <ListChecks className="h-4 w-4 text-amber-500" />
            {data.problems.filter((p) => p.type === "mcq").length} MCQ
          </span>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {data.problems.map((p) => {
            const dc = difficultyConfig[p.difficulty] ?? difficultyConfig.easy
            return (
              <Link key={p._id} href={`/problems/${p.slug}`} className="group block">
                <Card className="h-full overflow-hidden transition-all duration-200 group-hover:-translate-y-0.5 group-hover:border-orange-300 group-hover:shadow-md dark:group-hover:border-orange-500/50">
                  <div className={`h-1 w-full ${dc.stripe}`} />
                  <CardContent className="flex h-full flex-col gap-3 p-5">
                    <div className="flex items-center justify-between gap-2">
                      <Badge tone={dc.tone}>{p.difficulty}</Badge>
                    ...
                    {p.contestId && (
                      <p className="text-xs text-muted-foreground">
                        From &ldquo;{p.contestId.title}&rdquo;
                      </p>
                    )}
                    {p.type === "coding" && (
