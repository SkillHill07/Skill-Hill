...
export function ContestCard({ contest, participants }: { contest: ContestCardData; participants?: number }) {
  ...
  return (
    <Link href={`/contests/${contest._id}`} className="group block">
      <Card className="transition-all duration-200 group-hover:-translate-y-0.5 group-hover:border-indigo-300 group-hover:shadow-md dark:group-hover:border-indigo-500/50">
        <CardContent className="flex flex-col gap-3 p-5">
          <div className="flex items-center justify-between gap-2">
            <ContestStatusBadge status={contest.status} />
            <div className="flex items-center gap-2">
              {contest.type === "paid" && (
                <Badge tone="violet">{inr(contest.entryFee)} entry</Badge>
              )}
